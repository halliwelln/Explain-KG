...
for rule,trace_length in d.items():
    ...
    data = np.load(os.path.join('..','reproduce_results','data',DATASET+'.npz'))
    
    pred_data = np.load(
        os.path.join('..','reproduce_results','data','preds',DATASET,METHOD+'_'+DATASET+'_'+rule+'_preds.npz'),
        allow_pickle=True
    )
    ...
    #fig.tight_layout()
    plt.savefig(os.path.join('..','reproduce_results','data','plots',
        f"{DATASET}_{METHOD}_{rule}_counts.pdf",bbox_inches='tight'))
